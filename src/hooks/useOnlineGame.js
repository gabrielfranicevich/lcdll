...
export const useOnlineGame = (setScreen, mySessionId, localIp, playerName) => {
  ...
  const [myHand, setMyHand] = useState([]); // Player's white cards
  ...
  useEffect(() => {
    ...
    const handleSubmissionAccepted = ({ submissionId }) => {
      console.log('My submission accepted:', submissionId);
    ...
    socket.on('gameStarted', (data) => {
      setMySubmissionId(null); // Reset on new game
    ...
    socket.on('submissionAccepted', handleSubmissionAccepted);
  ...
  const startOnlineGame = useCallback(() => {
    if (socket && isHost && roomId && roomData) {
      // For card-based game, just tell the server to start
      // The server already has the cards loaded from cartas.json
  ...
  const submitVote = useCallback((submissionId) => {
    if (socket && roomId) {
      // Send singular submissionId to match server expectation
      socket.emit('submitVote', { roomId, submissionId });
    }
  }, [socket, roomId]);
  ...
  const submitCards = useCallback((cards) => {
    if (socket && roomId) {
      socket.emit('submitCards', { roomId, cards });
    }
  }, [socket, roomId]);
