 ...
 * @param {Array<Array<string>>} words - tuple of arrays of strings ([negras, blancas])
...
export const validateWordList = (words) => {
  ...
  if (!Array.isArray(words) || !Array.isArray(words[0]) || !Array.isArray(words[1])) {
    errors.push('La lista debe ser un arreglo de 2 arreglos de palabras');
    return { valid: false, errors };
  }

  if (words[0].length === 0 || words[1].length === 0) {
    errors.push('La lista debe contener al menos una palabra en cada arreglo');
    return { valid: false, errors };
  }

  const invalidWords = words[0].filter(w => !w || typeof w !== 'string' || w.trim() === '') +
    words[1].filter(w => !w || typeof w !== 'string' || w.trim() === '');
